...
public class ExecStopServerHandler : IRequestHandler<ExecStopServerCommand>
{
    ...
    private readonly ICrazyReport<ExecStopServerHandler> _crazyReport;
    private readonly IDispatcher _dispatcher;

    public ExecStopServerHandler(ILifecycleServices lifecycleServices,
        INotificationService notificationService,
        ICrazyReport<ExecStopServerHandler> crazyReport, IDispatcher dispatcher)
    {
    ...
    public Task Handle(ExecStopServerCommand request, CancellationToken ct)
        => request
            .HandleWithData(request, Exec, OnFailure)
            .HandleExceptionFor<WebServiceException>(OnFailure)
            .ExecAsync(ct);

    private Task Exec(ExecStopServerCommand request, CancellationToken ct)
